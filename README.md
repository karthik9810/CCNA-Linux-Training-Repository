# CCNA-Linux-Training-Repository
"A 20-day learning repository for CCNA and Linux with structured lessons, practical exercises, and labs. Covers networking fundamentals, routing, switching, Linux commands, shell scripting, and real-world practice for beginners and intermediate learners."
linux-20-days-roadmap/






│
├── Day01_Basics/
├── Day02_File_System/
├── Day03_File_Commands/
├── Day04_Permissions/
├── Day05_Text_Editors/
├── Day06_Package_Management/
├── Day07_Users_Groups/
├── Day08_Process_Management/
├── Day09_Networking/
├── Day10_Disk_Management/
├── Day11_Boot_System/
├── Day12_Logs_Monitoring/
├── Day13_Shell_Scripting/
├── Day14_Shell_Scripting_Advanced/
├── Day15_Cron_At/
├── Day16_System_Security/
├── Day17_SSH_SCP/
├── Day18_Archiving_Backups/
├── Day19_Troubleshooting/
└── Day20_Mini_Project/
